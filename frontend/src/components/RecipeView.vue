      ...
      <div class="p-6 sm:p-8 border-b border-amber-100 bg-gradient-to-r from-amber-50 to-white">
        <div class="flex justify-between items-start">
          <div>
            ...
            <div class="mt-2 inline-block bg-amber-100 px-3 py-1 rounded-full">
              <span class="text-amber-700 font-medium">Por </span>
              <span class="text-amber-800 font-semibold">{{ receta.user_name }}</span>
            </div>
...
const submitCommentReport = async () => {
  ...
  try {
    ...
    if (response.data.message) {
      alert('Reporte enviado correctamente. Gracias por ayudar a mejorar la comunidad.');
    }
  } catch (error) {
    console.error("Error al reportar:", error);
    alert('Ocurrió un error al enviar el reporte. Por favor intenta nuevamente.');

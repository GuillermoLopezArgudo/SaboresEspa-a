  ...
  <div class="min-h-screen bg-amber-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="elementos.recetas.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="(item) in elementos.recetas" :key="item.id" class="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 border border-amber-100">
          <!-- Imagen de la receta (puedes agregarla si tienes) -->
          <div class="h-48 bg-amber-200 flex items-center justify-center text-amber-600">
          ...
          <div class="p-5">
            <!-- Botón de favoritos -->
            <button @click="toggleFavorite(item.id)" class="mb-4 flex items-center space-x-2 transition-all duration-200 hover:scale-105">
              <i class="fa text-2xl" :class="isFavorite(item.id) ? 'fa-heart text-red-500' : 'fa-heart-o text-amber-400 hover:text-red-400'"></i>
              <span class="font-medium text-amber-700">Guardar en favoritos</span>
            </button>
      ...
      <div v-else class="text-center py-12">
        <div class="max-w-md mx-auto">
          ...
          <div class="mt-6">
            <router-link to="/add-recipe" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">

<template>
    <div class="container mx-auto mt-4 px-4">
      ...
      <router-link to="/home" class="btn btn-secondary mb-3 inline-block px-4 py-2 text-white bg-gray-500 rounded hover:bg-gray-600">Atrás</router-link>
      <form @submit.prevent="submitRecipe" class="shadow-lg p-6 rounded-lg bg-white">
        <div class="mb-4">
          ...
          <input type="text" id="titleRecipe" v-model="title" class="form-control mt-2 p-2 w-full rounded border border-gray-300 focus:ring-blue-500" required />
        </div>
        <div class="mb-4">
          <label for="imageRecipe" class="block text-lg font-semibold">Imagen de la receta</label>
          <input type="file" id="imageRecipe" @change="handleImageChange" class="form-control mt-2 p-2 w-full rounded border border-gray-300 focus:ring-blue-500" required />
        </div>
        <div class="mb-4">
          <label for="videoRecipe" class="block text-lg font-semibold">Video de la receta (Opcional)</label>
          <input type="file" id="videoRecipe" @change="handleVideoChange" class="form-control mt-2 p-2 w-full rounded border border-gray-300 focus:ring-blue-500" />
        </div>
        <div class="mb-4">
          <label for="descriptionRecipe" class="block text-lg font-semibold">Descripción de la receta</label>
          <textarea id="descriptionRecipe" rows="5" v-model="description" class="form-control mt-2 p-2 w-full rounded border border-gray-300 focus:ring-blue-500" required></textarea>
        </div>
        <div class="mb-4">
          ...
          <div v-for="(ingredient, index) in recipebook.ingredients" :key="index" class="flex mb-2">
            <input v-model="recipebook.ingredients[index]" placeholder="Ingrediente" class="form-control p-2 w-1/2 rounded border border-gray-300 focus:ring-blue-500 mr-2" />
            <input v-model="recipebook.quantities[index]" placeholder="Cantidad" class="form-control p-2 w-1/2 rounded border border-gray-300 focus:ring-blue-500" />
          </div>
        </div>
        <div class="mb-4">
        ...
        <div class="d-grid mt-6">
          <input type="submit" value="Crear Receta" class="w-full py-3 bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-500" />
        </div>
      </form>
    </div>
  </template>
